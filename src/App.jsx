...
>
  🗑️ Supprimer
          ...
          {coachTab === "connections" && <section className="season-list coach-connection-section"><div className="section-heading"><div><span className="card-label">CONNEXION</span><h2>Inviter un athlète</h2></div></div><div className="profile-form"><p className="coach-connection-intro">Génère un code et donne-le à ton athlète. Il pourra le saisir depuis son espace <span translate="no">Season</span>.</p>{coachInvitationCode ? <div style={{ marginTop: "20px", padding: "25px", borderRadius: "18px", background: "rgba(0,0,0,0.04)", textAlign: "center" }}><span className="card-label">TON CODE D'INVITATION</span><div style={{ fontSize: "36px", fontWeight: 800, letterSpacing: "6px", margin: "15px 0" }}>{coachInvitationCode}</div><div style={{ display: "flex", justifyContent: "center", gap: "10px", flexWrap: "wrap" }}><button type="button" className="primary-button" onClick={copyInvitationCode}>📋 Copier le code</button><button type="button" className="outline-button" onClick={generateCoachInvitationCode} disabled={loadingInvitationCode}>+ Nouveau code</button></div><p style={{ marginTop: "15px", fontSize: "14px", opacity: 0.65 }}>Un code devient inutilisable lorsqu'un athlète l'utilise.</p></div> : <button type="button" className="large-submit coach-connection-button" onClick={generateCoachInvitationCode} disabled={loadingInvitationCode}>{loadingInvitationCode ? "Génération..." : "Générer mon code d'invitation →"}</button>}{invitationMessage && <div className={invitationMessage.startsWith("Erreur") ? "auth-message" : "success-message"}>{invitationMessage}</div>}</div></section>}
